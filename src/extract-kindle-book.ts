  ...
  try {
    ...
    if (DBG) {
      page.on('console', (msg: ConsoleMessage) =>
        dlog('[browser]', msg.type(), msg.text())
      )
      page.on('requestfailed', (req: Request) =>
        dlog('[requestfailed]', req.failure()?.errorText, req.url())
      )
    }
    ...
    async function openTableOfContents() {
      ...
      const directCandidates: Locator[] = [
        ...
        scope.locator?.(
          '[aria-label="Table of Contents"], [data-testid="toc-button"]'
        )
      ].filter(Boolean) as Locator[]
      ...
      while (!opened && Date.now() < deadline) {
        for (const cand of directCandidates) {
          if (await cand.isVisible().catch(() => false)) {
            ...
            await page
              .locator('ion-menu, .side-menu, .toc, ion-list')
              .first()
      ...
      if (!opened) {
        ...
        const menuBtn = scope.locator?.(
          'ion-button[title="Reader menu"], button[title="Reader menu"], [aria-label="Reader menu"]'
        )
        if (
          menuBtn &&
          (await menuBtn
        ...
        const menuItems: Locator[] = [
          scope.locator?.(
            'ion-item[role="listitem"]:has-text("Table of Contents")'
          ),
          scope.locator?.('ion-item[role="listitem"]:has-text("Contents")'),
          scope.getByRole?.('menuitem', {
            name: /table of contents|contents/i
          } as any)
        ].filter(Boolean) as Locator[]
      ...
      if (!opened) {
        await page
          .screenshot({ path: 'toc-open-timeout.png', fullPage: true })
          .catch(() => {})
        throw new Error(
          'Could not open Table of Contents. Saved screenshot: toc-open-timeout.png'
        )
      }
    ...
    async function goToPage(pageNumber: number) {
      ...
      if (LOCATION_MODE) {
        const navInitial = await getPageNav(false).catch(
        ...
        const stepOnce = async (
          ...
          const chevronSel =
            direction === 'forward' ? SEL.chevronRight : SEL.chevronLeft
        ...
        while (currentNav?.page !== pageNumber && iterations < maxIterations) {
          ...
          if (absDelta > 50 && tocAttempts < 5) {
            const jumped = await attemptTocJump(
              direction,
              currentNav,
              pageNumber
            )
      ...
      const menuCandidates: Locator[] = [
        ...
        scope.locator?.(
          '[aria-label="Menu"], [data-testid="reader-menu"], ion-button:has(ion-icon[name="menu"])'
        )
      ].filter(Boolean) as Locator[]
      ...
      while (!menuOpened && Date.now() < openDeadline) {
        for (const cand of menuCandidates) {
          if (await cand.isVisible().catch(() => false)) {
            ...
            const menuList = scope.locator?.(
              'ion-list, [role="menu"], .menu-content'
            )
            await menuList
              ?.first()
            ...
            const gotoItem = scope.locator?.(
              'ion-item[role="listitem"]:has-text("Go to Page"), [role="menuitem"]:has-text("Go to Page")'
            )
            if (
              gotoItem &&
              (await gotoItem
      ...
      if (!menuOpened) {
        await page
          .screenshot({ path: 'goto-open-menu-timeout.png', fullPage: true })
          .catch(() => {})
        throw new Error(
          'Could not open Reader menu to navigate to a page. Saved screenshot: goto-open-menu-timeout.png'
        )
      }
      ...
      if (
        ...
        await page
          .screenshot({ path: 'goto-item-missing.png', fullPage: true })
          .catch(() => {})
        throw new Error(
          '"Go to Page" menu item not found. Saved screenshot: goto-item-missing.png'
        )
      }
      await gotoItem
        .first()
      ...
      const inputBox = scope.locator?.(
        'ion-modal input[placeholder="page number"], ion-modal input[type="text"], [role="dialog"] input'
      )
      if (!inputBox) {
        await page
          .screenshot({ path: 'goto-input-missing.png', fullPage: true })
          .catch(() => {})
        throw new Error(
          'Go to Page input not found. Saved screenshot: goto-input-missing.png'
        )
      }
      await inputBox
        .first()
        .fill(String(pageNumber), { timeout: 2000 })
        .catch(() => {})

      // Click Go / submit or press Enter
      const goBtn = scope.locator?.(
        'ion-modal ion-button[item-i-d="go-to-modal-go-button"], ion-modal button:has-text("Go")'
      )
      if (
        goBtn &&
        (await goBtn
      ...
      if (!nav || nav.page !== pageNumber) {
        ...
        while (
          ...
          await delay(100)
          const refreshedNav = await getPageNav(false).catch(
            () => undefined as PageNav | undefined
          )
        ...
        if (nav?.page !== pageNumber) {
          ...
          const reOpenMenu = [
            ...
            scope2.getByRole?.('button', {
              name: /reader menu|menu|more/i
            } as any),
            scope2.locator?.(
              '[aria-label="Menu"], [data-testid="reader-menu"], ion-button:has(ion-icon[name="menu"])'
            )
          ...
          const gotoItem2 = scope2.locator?.(
            'ion-item[role="listitem"]:has-text("Go to Page"), [role="menuitem"]:has-text("Go to Page")'
          )
          if (
            gotoItem2 &&
            ...
            await modal2
              ...
              .waitFor({ state: 'visible', timeout: 2000 })
              .catch(() => {})
            const input2 = scope2.locator?.(
              'ion-modal input[placeholder="page number"], [role="dialog"] input'
            )
            if (input2) {
              await input2
                .first()
          ...
          while (Date.now() < finalWaitDeadline) {
            const finalNav = await getPageNav(false).catch(
              () => undefined as PageNav | undefined
            )
            nav = finalNav ?? nav
            if (nav?.page === pageNumber) break
        ...
        if (nav?.page !== pageNumber) {
          await page
            .screenshot({ path: 'goto-fallback-failed.png', fullPage: true })
            .catch(() => {})
          throw new Error(
            `Failed to navigate to page ${pageNumber}. Saved screenshot: goto-fallback-failed.png (last seen page ${nav?.page ?? 'unknown'})`
          )
        }
      }
    ...
    try {
      ...
      if (
        rawParsed &&
        rawParsed.page === undefined &&
        rawParsed.location !== undefined
      ) {
        LOCATION_MODE = true
      }
    } catch {}
    ...
    for (const tocItem of $tocItems) {
      ...
      if (!pageNav) {
        throw new Error(
          'Failed to read page navigation while collecting TOC items'
        )
      }
    ...
    assert(
      firstPageTocItem,
      'Unable to find first valid page in TOC (post-parse)'
    )
    ...
    async function isEndState() {
      try {
        ...
        const footerText =
          (await footer.textContent().catch(() => null))?.trim() || ''
        const parsed = parsePageNav(footerText)
        const nearEnd =
          parsed?.page !== undefined &&
          parsed?.total !== undefined &&
          parsed.page >= parsed.total - 1
        const hasFooter = !!parsed?.page || !!parsed?.location
        if (DBG)
          dlog('isEndState:', { hasChevron, footerText, parsed, nearEnd })
    ...
    async function advanceOnePageFromStuck(
      prevSrc?: string,
      ...
      if (box) {
        await page.mouse
          .click(box.x + box.width * 0.85, box.y + box.height * 0.5)
          .catch(() => {})
        await delay(220)
        const newSrc = await img.getAttribute('src').catch(() => undefined)
        if (prevSrc && newSrc && newSrc !== prevSrc) return true
    ...
    while (!reachedEnd) {
      ...
      const src = await page.locator(SEL.mainImg).getAttribute('src')
      dlog(
        'loop: at page',
        pageNav.page,
        'of',
      ...
      if (prevEntry && prevEntry.page === pageNav.page) {
        ...
        const advanced = await advanceOnePageFromStuck(
          src ?? undefined,
      ...
      if (stagnantCount >= 2) {
        console.warn(
          'no progress after multiple iterations; assuming end and exiting'
        )
        reachedEnd = true
        break
      }
      ...
      if (await isEndState()) {
        ...
        const finalPath = path.join(
          pageScreenshotsDir,
          `${finalIndex}`.padStart(pagePadding, '0') +
            '-' +
            `${labeledPage}`.padStart(pagePadding, '0') +
            '.png'
        )
      ...
      do {
        ...
        try {
          if (LOCATION_MODE) {
            ...
            if (!advanced && pageNav.total - pageNav.page <= 1) {
              ...
              console.warn(
                'end-of-book reached or next page control unavailable; stopping'
              )
              retries = maxRetries
              break
            }
        ...
        while (Date.now() - startWait < 1500) {
          ...
          const navNow = await getPageNav(false).catch(
            () => undefined as PageNav | undefined
          )
      ...
      if (
        afterPage === undefined ||
        afterPage === pageNav.page ||
        afterPage >= totalContentPages
      ) {
        reachedEnd = true
      }
    }
    ...
    if (initialPageNav?.page !== undefined) {
      ...
      if (SKIP_RESET_FLAG || DBG) {
        console.warn(
          `skip reset enabled — leaving reader at page ${endedOn ?? 'unknown'}`
        )
      } else if (endedOn === initialPageNav.page) {
        console.warn(
          `already on initial page ${initialPageNav.page}; skipping reset`
        )
      } else {
